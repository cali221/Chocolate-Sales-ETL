...
def update_random_orders_status(host, port):
    # get uncompleted orders (limit 100)
    orders = requests.get(f"http://{host}:{port}/uncompleted_orders").json()
    ...
    if not orders:
        ...
        all_statuses = requests.get(f"http://{host}:{port}/status").json()
